---
title: "Signatures of positive selection at drug resistance loci in *Mycobacterium tuberculosis*"
...
Using genes under selection for drug resistance
### Study motivation
 in *M. tuberculosis*, we aim to test the ability of population genetics statistics to identify genes under positive selection in bacteria with strictly clonal reproduction.

### Data set description

We downloaded sequencing read data from two large surveys of drug resistant *M. tuberculosis* in Russia (Casali et al. 2014) and South Africa (Cohen et al. 2015). Reads were mapped to *M. tuberculosis* H37Rv using our validated reference guided assembly pipeline (https://github.com/pepperell-lab/RGAPepPipe/). We removed isolates with mean coverage less than 20X, isolates with percentage of the genome covered at 10X less than 90%, isolates where a majority of reads did not map to H37Rv, and isolates where greater than 10% of sites were unknown after mapping. The final dataset contains 1161 M. tuberculosis isolates, primarily from lineage 2 and lineage 4. The alignment was masked to remove repetitive regions including PE/PPE genes.

### Phylogenetic analysis

Phylogenetic analysis was performed using FastTree v 2.1.9 (compiled with the double precision option). 

```{r tree, include=TRUE}

fasttree <- read.tree("../data/tree.newick")
tree_plot <- ggtree(fasttree, layout = "unrooted")
tree_plot
...
resGenesStats <- stats[stats$gene %in% resistanceGenes,]
...
resGeneStats_lin2 <- lineage2_stats[lineage2_stats$gene %in% resistanceGenes,]
...
resGeneStats_lin4 <- lineage4_stats[lineage4_stats$gene %in% resistanceGenes,]
...
stats_pi <- filter(stats_all.melt, grepl("pi", variable), !grepl("pr", variable))
...
```
`r kable(drug_resistant_residuals, caption = "Tajima's D and residual values of drug resistance loci")`
...
Resistant isolates have more genes with no diversity compared to susceptible isolates. However, among genes with measurable diversity, values between susceptible and resistant isolates are similar.

####Shared invariant genes among drug resistant populations
...
Drug resistant populations have more invariant genes than susceptible populations, and the subset of invariant genes is larger than expected if genes are randomly selected as invariant.
...
Homoplasy has been previously used to identify drug resistance associated loci in M. tb (Farhat et al. 2013). We identified homoplasies by performing ancestral reconstruction in TreeTime and counting the number of times SNPs arose on the phylogeny. Homoplastic SNPs were significantly enriched in drug resistance genes.
...
`r kable(sum.table.dr, caption= "Signatures of selection in known drug resistance genes.")`
`r kable(fst_homoplasy_table_all, caption = "Homoplastic Fst outliers")`